...
open import Base
open import NConnected
open import HSpace renaming (HSpaceStructure to HSS)
open import SuspensionType
import WedgeExtension as WedgeExt
open import TwoSemiCategoryType
open import Functor
open import FundamentalCategory

module Pi2HSuspCompose {i} {X : Ptd i} {{_ : has-level (S (S (S ⟨-2⟩))) (de⊙ X)}}
  {{is-0-connected : is-connected (S (S ⟨-2⟩)) (de⊙ X)}} (H-X : HSS X)
  ...
  open import Pi2HSusp H-X public

  infixr 80 _∙₁_
  _∙₁_ : {x y z : Susp A} → Trunc (S (S (S ⟨-2⟩))) (x == y) → Trunc (S (S (S ⟨-2⟩))) (y == z) → Trunc (S (S (S ⟨-2⟩))) (x == z)
  ...
  comp-args =
    record {
      ...
      f = comp-r₁;
      g = comp-l₁;
      p = admit {-ap (λ {(p₁ , p₂) → ap [_] (ap η p₁ ∙ p₂)})
  ...
  module CoherenceProof (a' : A) where
    ...
    P-is-prop : ∀ a a'' → has-level (S ⟨-2⟩) (P a a'')
    P-is-prop a a'' = has-level-apply (has-level-apply (Trunc-level {n = S (S (S ⟨-2⟩))}) _ _) _ _
  ...
  comp-functor : (pentagon : coh-assoc-pentagon H-X H-X-assoc)
    → TwoSemiFunctor
        (HSpace-2-semi-category H-X {{admit}} H-X-assoc pentagon)
