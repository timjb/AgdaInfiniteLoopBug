...
inhab-conn : ∀ {i} {A : Type i} (a : A) → is-connected (S (S ⟨-2⟩)) A
...
conn-extend c P f = admit -- is-equiv.g (pre∘-conn-is-equiv c P) f
...
conn-extend-β c P f = admit -- app= (is-equiv.f-g (pre∘-conn-is-equiv c P) f)
...
conn-extend-general c P t = admit {-
...
abstract
  ...
  pointed-conn-out {n = n} A a₀ {{c}} a = admit {-has-level-in
    (point ,
     λ y → ! (cancel point)
