...
module CupProduct01 {i} (R : CRing i) where
  ...
  module distr-l (g₁ g₂ : R.El) where
    ...
    abstract
      ...
      loop' h = admit {-↓-='-in {f = f} {g = g} {p = emloop h} {u = base'} {v = base'} $
        base' ∙' ap g (emloop h)
  ...
  group-to-EM₁-endos =
    ...
    ; pres-comp-coh = admit -- pres-comp-coh
    }
    {-
    where
    abstract
  ...
  group-to-EM₁→EM₂-op =
    comp-semi-cat-functors {C = group-to-cat R₊}
                           ...
                           group-to-EM₁-endos
                           (fun-functor-map (EM₁ R₊) {G = EM₁-2-semi-category}
                                                     {H = dual-cat (=ₜ-fundamental-cat (Susp (EM₁ R₊)))}
                                                     comp-functor)
