...
record AlternativeHSpaceStructure {i} (X : Ptd i) : Type i where
  ...
  ⊙μ : (X ⊙× X) ⊙→ X
  ⊙μ = (λ p → μ (fst p) (snd p)) , unit-l (pt X)
...
module ConnectedHSpace {i} {X : Ptd i} {{_ : is-connected (S (S ⟨-2⟩)) (de⊙ X)}}
...
module _ {i} {X : Ptd i} (hX : HSpaceStructure X) where
  ...
  HSpace-2-semi-category : {{X-level : has-level (S (S (S ⟨-2⟩))) (de⊙ X)}}
    → (assoc : associator)
    → coh-assoc-pentagon assoc
    → TwoSemiCategory lzero i
  HSpace-2-semi-category {{X-level}} assoc assoc-coh =
    record
    { El = ⊤
    ; Arr = λ _ _ → de⊙ X
    ; Arr-level = λ _ _ → X-level
    ; two-semi-cat-struct =
      record
      { comp = hX.μ
      ; assoc = assoc
